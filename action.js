/**
 * Finish line estimator. This tool will provide better context of running
 * each job by calculating the finish date and time. This primarily 
 * relies on the quality of information provided by the user. The more
 * accurate the inputs, the more precise the output will be.
 */
 ...
 *      Also makes sure that the ending sequence is not greater than 
 *      the starting.
 ...
 */
let finishlineEstForm = document.getElementById('finishLine-input');
finishlineEstForm.addEventListener("submit", (e) => {
    e.preventDefault();
    calcTotalQty();
    calcProdRun();
    calcRollChange();
...
/**
 * Velocity suggestion tool. This will calculate the optimal sheets per
 * hour goal to achieve the supplied estimate. This also helps to find 
 * where we can cut the actual runtime by providing a realistic speed
 * goal that can be used across all shifts and operators. 
 */
...
let sheetCount = document.getElementById("pcPerSht");
let sheetCountValue = 0;

let calculatedVelo = 0.00;

function calcVelo(){
    calculatedVelo = Math.round((ticketPieceValue / sheetCountValue) / ticketRunValue);
}
...
function getPcPerShtCount() {
    sheetCountValue = sheetCount.value;
}
...
function displayResult() {
    ...
    document.getElementById("velocity").innerHTML = calculatedVelo+ " sheets / hour";
}
...
/**
 * Roll estimator tool. This tool is still a work-in-progress, but will
 * take the number of sheets on a roll and calculate against the total 
 * piece count the number of rolls the job may have. This is slightly
 * less precise and should give a close roll count. 
 */
